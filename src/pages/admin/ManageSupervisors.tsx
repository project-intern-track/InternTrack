...
const ManageSupervisors = () => {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-gray-50 via-orange-50/40 to-gray-50 p-6 md:p-8">
            ...
            >
                <div>
                    <h1 className="text-3xl font-extrabold tracking-tight text-gray-900">Manage Supervisors</h1>
                    <p className="mt-1 text-sm text-gray-600">Manage supervisor accounts, status, and profile details.</p>
                </div>
                <button
            ...
            <div className="mb-7 grid grid-cols-1 gap-4 md:grid-cols-3">
                {[
                    ...
                    { label: 'Active Supervisors', value: stats.activeSupervisors },
                    { label: 'Archived Supervisors', value: stats.archivedSupervisors },
                ].map((item, index) => (
                    <motion.div
                        key={item.label}
                        ...
                        <tbody>
                            {paginatedSupervisors.length === 0 ? (
                                ...
                                paginatedSupervisors.map((supervisor, index) => (
                                    ...
                                    >
                                        <td className="px-5 py-4 font-medium text-gray-900">{supervisor.full_name}</td>
                                        <td className="px-5 py-4">{supervisor.email}</td>
                                        <td className="px-5 py-4">{formatDate(supervisor.created_at)}</td>
                                        <td className="px-5 py-4">
                                            <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${supervisor.status === 'active' ? 'bg-emerald-50 text-emerald-700' : 'bg-violet-50 text-violet-700'}`}>
                                                {supervisor.status}
                                            </span>
                                        </td>
                                        <td className="px-5 py-4">
                    ...
                    >
                        ...
                        >
                            <h2 className="mb-5 text-xl font-bold text-gray-900">
                                {confirmationStep ? 'Confirm Supervisor Addition' : 'Add New Supervisor'}
                            </h2>
                            ...
                            {!confirmationStep ? (
                                ...
                                <div className="rounded-xl border border-orange-100 bg-orange-50/60 p-4">
                                    <p className="text-sm text-gray-700">
                                        Are you sure you want to upgrade <span className="font-semibold text-gray-900">{selectedInternName}</span> to supervisor?
                                    </p>
                                </div>
                            )}
                ...
                {editingSupervisor && (
                    ...
                    >
                        <motion.div
                            ...
                            onClick={(e) => e.stopPropagation()}
                        >
                            <h2 className="mb-5 text-xl font-bold text-gray-900">Edit Supervisor Information</h2>
