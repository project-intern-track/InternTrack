...
const AdminDailyLogs = () => {
    ...
    // ── Fetch own logs + today's record ───────────────────────────────────
    const refresh = useCallback(async () => {
        setError(null);
        try {
            // Pass user_id so admin only sees their own records (not all interns')
            const [all, today] = await Promise.all([
                attendanceService.getAttendance(user?.id ? { user_id: user.id } : undefined),
        ...
        <div className="p-6 space-y-6">
            ...
            <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                    Track your personal OJT attendance
                </p>
            </div>
